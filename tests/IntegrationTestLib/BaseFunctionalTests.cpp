...
#ifdef _WIN32
const std::string DAEMON_FILENAME = "chavezcoind.exe";
#else
const std::string DAEMON_FILENAME = "chavezcoind";
#endif
...
void BaseFunctionalTests::startNode(size_t index) {
  ...
  config
    ...
    << "log-file=test_chavezcoind_" << index << ".log" << std::endl;
...
#if defined WIN32
  std::string commandLine = "start /MIN \"chavezcoind" + std::to_string(index) + "\" \"" + daemonPath.string() +
  ...
  if (pid == 0) {
    ...
    if (execl(pathToDaemon.c_str(), "chavezcoind", "--testnet", dataDir.c_str(), "--config-file=daemon.conf", NULL) == -1) {
